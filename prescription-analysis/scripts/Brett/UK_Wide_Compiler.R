...
for(i in 1:length(unique(data_York$API))){
  if (i == 1) {
    write.xlsx(data_York[data_York$API == unique(data_York$API)[i],], file = paste(Sys.Date(),class,p,".xlsx", sep = ""), sheetName = unique(data_York$API)[i], row.names = FALSE)
  } else {
    write.xlsx(data_York[data_York$API == unique(data_York$API)[i],], file = paste(Sys.Date(),class,p,".xlsx",sep=""), sheetName = unique(data_York$API)[i], append = TRUE, row.names = FALSE)
  }
}
}
# 
