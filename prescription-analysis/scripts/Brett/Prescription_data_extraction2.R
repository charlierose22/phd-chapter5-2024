...
#load the raw data
data_raw <- read.csv("1609_Detailed_Prescribing_Information.csv",header = TRUE, stringsAsFactors = FALSE, sep = ",")
practices <- read.csv("Practices.csv",header=FALSE,stringsAsFactors = FALSE, sep=",")
chems <- read.csv("API_BNF_BetaLactams.csv",header=FALSE,stringsAsFactors = FALSE, sep=",")
...
for (i in 1:length(data_York_sub)) {
  ...
  if (nrow(data_York_sub[[i]])>0) {
    ...
    #write to worksheet  
    if (i == 1) {
      write.xlsx(x = data_York_sub[[i]], file = paste(Sys.Date(),"_allAPIs",".xlsx",sep=""), sheetName = chems$V1[i], row.names = FALSE)
    } else {
      write.xlsx(x = data_York_sub[[i]], file = paste(Sys.Date(),"_allAPIs",".xlsx",sep=""), sheetName = chems$V1[i], append = TRUE, row.names = FALSE)
    }
  }
...
data_York_sub[[i]]$BNF.Description[data_York_sub[[i]]$BNF.Description=="Augmentin_Pdr For Susp 250/62mg/5ml S/F"] <- "Augmentin_Pdr For Susp 250mg/62mg/5ml S/F"
